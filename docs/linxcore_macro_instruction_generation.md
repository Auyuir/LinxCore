...
- `/Users/zhoubot/LinxCore/pyc/linxcore/common/decode.py`
- `/Users/zhoubot/LinxCore/pyc/linxcore/backend/code_template_unit.py`
- `/Users/zhoubot/LinxCore/pyc/linxcore/backend/backend.py`
...
- Frame template macros:
  ...
  - `FRET_STK`
  - `FRET_RA`
- Boundary-call macro behavior:
  - `BSTART_STD_CALL` with implicit `setret`-style architectural writeback event
...
`BSTART_STD_CALL` is treated as boundary control plus a `setret`-style RA writeback event in retire path.

Rule:

- RA update is represented as a regular architectural writeback event in retire trace (`wb_valid/wb_rd/wb_data`), not as a dedicated RA-only datapath.
