...
def build_janus_bcc_bctrl(m: Circuit) -> None:
    ...
    to_tau_bctrl = cmd_issue_bctrl & (~to_tma_bctrl) & (~to_cube_bctrl)
    ...
    rsp_valid_brob = rsp_tma_valid_tma | rsp_cube_valid_cube | rsp_tau_valid_tau
    rsp_tag_brob = rsp_cube_valid_cube._select_internal(rsp_cube_tag_cube, rsp_tau_tag_tau)
    rsp_tag_brob = rsp_tma_valid_tma._select_internal(rsp_tma_tag_tma, rsp_tag_brob)
    rsp_status_brob = rsp_cube_valid_cube._select_internal(rsp_cube_status_cube, rsp_tau_status_tau)
    rsp_status_brob = rsp_tma_valid_tma._select_internal(rsp_tma_status_tma, rsp_status_brob)
    rsp_data0_brob = rsp_cube_valid_cube._select_internal(rsp_cube_data0_cube, rsp_tau_data0_tau)
    rsp_data0_brob = rsp_tma_valid_tma._select_internal(rsp_tma_data0_tma, rsp_data0_brob)
    rsp_data1_brob = rsp_cube_valid_cube._select_internal(rsp_cube_data1_cube, rsp_tau_data1_tau)
