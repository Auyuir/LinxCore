...
constexpr const char *kDefaultIsaPy = "/Users/zhoubot/LinxCore/pyc/linxcore/common/isa.py";
...
int main(int argc, char **argv) {
  ...
  if (traceKonata) {
    ...
    if (const char *p = std::getenv("PYC_KONATA_PATH"); p && p[0] != '\0') {
      ...
      outPath = std::filesystem::path("/Users/zhoubot/LinxCore/generated/cpp/linxcore_ooo_pyc") /
                ("tb_linxcore_ooo_pyc_cpp_" + (stem.empty() ? std::string("program") : stem) + ".konata");
    }
