...
if [[ ! -f "${GEN_HDR}" ]]; then
  need_regen=1
elif find "${ROOT_DIR}/pyc/linxcore" "${ROOT_DIR}/pyc/linxcore_ooo_pyc.py" -name '*.py' -newer "${GEN_HDR}" | grep -q .; then
  need_regen=1
fi
if [[ "${need_regen}" -ne 0 ]]; then
  echo "[cosim] generating LinxCore OOO PYC artifacts"
...
RANGES="$(${ROOT_DIR}/scripts/build_sparse_ranges.py --elf "${ELF}" --boot-sp "${BOOT_SP}" --force-et-rel)"
